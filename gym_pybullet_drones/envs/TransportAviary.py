...
class TransportAviary(gym.Env):
    ...
    def _housekeeping(self):
        ...
        self.ROD_IDS = np.array([p.loadURDF(pkg_resources.resource_filename('gym_pybullet_drones', 'assets/connecting_rod.urdf'),
                                    self.INIT_XYZS[i,:] + np.array([0, 0, -0.26]),  # 棒の初期位置をドローンの下方に設定
                                    p.getQuaternionFromEuler([0, 0, 0]),  # 棒の初期姿勢を設定
                                    physicsClientId=self.CLIENT
                                    ) for i in range(self.NUM_DRONES)])
        ...
        for i in range(self.NUM_DRONES):
            p.createConstraint(parentBodyUniqueId=self.DRONE_IDS[i],
                            parentLinkIndex=-1,
                            childBodyUniqueId=self.ROD_IDS[i],
                            childLinkIndex=0,  # 棒の上端のリンクインデックス
                            jointType=p.JOINT_POINT2POINT, 
                            jointAxis=[0, 0, 0],
                            parentFramePosition=[0, 0, 0],
                            childFramePosition=[0, 0, 0],  # 棒の長さの半分だけ下方にオフセット
                            physicsClientId=self.CLIENT)
        
        for i in range(self.NUM_DRONES):
            p.createConstraint(parentBodyUniqueId=self.BOX_ID,
                            parentLinkIndex=i,
                            childBodyUniqueId=self.ROD_IDS[i],
                            childLinkIndex=-1,
                            jointType=p.JOINT_POINT2POINT, 
